...
(function (global, factory) {
    ...
    function dispatchNativeEvent(el, value) {
      ...
      const nativeInputValueSetter = Object.getOwnPropertyDescriptor(prototype, 'value');
    ...
    /**
     * Query all elements matching a selector, including those inside Shadow DOM.
    ...
    function querySelectorAllDeep(selector, root = document) {
      ...
      // 2. Recursively find shadow roots and query them
    ...
    class DOMAnalyzer {
      ...
      _scanAll() {
        ...
        querySelectorAllDeep(aiSelector).forEach(el => this._register(el));
      }
      _register(el) {
        ...
        this._observer = new MutationObserver(mutations => {
          for (const mutation of mutations) {
            ...
            mutation.addedNodes.forEach(node => {
              ...
              const aiSelector = [...ALL_SDK_CLASSES].map(c => `.${c}`).join(',');
              querySelectorAllDeep(aiSelector, node).forEach(el => this._register(el));
            });
            // Removed nodes
    ...
    function findField(targetLabel, domAnalyzer) {
      ...
      // 2) Fallback: scan all input/textarea/select elements (including Shadow DOM)
      const allFields = querySelectorAllDeep('input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="reset"]):not([type="checkbox"]):not([type="radio"]), textarea, select');
    ...
    function collectTargets(domAnalyzer) {
      ...
      // Semantic fallbacks (including Shadow DOM)
      querySelectorAllDeep('section, article, main, header, footer, nav, [id]').forEach(addEl);
      return targets;
    }
    ...
    function findSearchInput(domAnalyzer) {
      ...
      // type="search" (including Shadow DOM)
      const typeSearch = querySelectorAllDeep('input[type="search"]')[0];
      if (typeSearch) return typeSearch;

      // Name / id / placeholder hints (including Shadow DOM)
      const hinted = querySelectorAllDeep('input[name*="search"], input[name*="query"], input[name*="q"],' + 'input[id*="search"], input[id*="query"],' + 'input[placeholder*="search" i], input[placeholder*="find" i]')[0];
      if (hinted) return hinted;

      // Fallback: role="search" container's first input (including Shadow DOM)
      const searchRole = querySelectorAllDeep('[role="search"] input')[0];
    ...
    function collectClickTargets(domAnalyzer) {
      ...
      // All links and buttons (including Shadow DOM)
      querySelectorAllDeep('a[href], button, [role="button"], [role="link"], input[type="button"], input[type="submit"]').forEach(addEl);
      return targets;
    }
